...
$pdo->exec("
  CREATE TABLE IF NOT EXISTS lecture_codes (
    ...
    CONSTRAINT fk_lecture_codes_lecture FOREIGN KEY (lecture_id) REFERENCES lectures(id) ON DELETE SET NULL
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
");
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
  ...
  if ($q !== '') {
    $where .= " AND (lc.code LIKE ?)";
    $params[] = '%' . $q . '%';
  }
  if ($filterUsed === '0' || $filterUsed === '1') {
    $where .= " AND lc.is_used = ?";
  ...
  $filename = 'اكواد_المحاضرات_' . date('Y-m-d_H-i') . '.xls';
  ...
  <head>
    <meta charset="utf-8">
    <title>تصدير أكواد المحاضرات</title>
    <style>
      ...
      <tbody>
        <?php $i=0; foreach ($rows as $r): $i++; ?>
          <tr>
            ...
            <td><?php echo ((int)$r['is_global'] === 1) ? 'عام (يفتح كل المحاضرات)' : 'مخصص لمحاضرة'; ?></td>
            <td><?php echo ((int)$r['is_global'] === 1) ? '—' : h((string)($r['course_name'] ?? '')); ?></td>
            <td><?php echo ((int)$r['is_global'] === 1) ? '—' : h((string)($r['lecture_name'] ?? '')); ?></td>
...
if (($_POST['action'] ?? '') === 'generate') {
  ...
  if (!$error) {
    ...
    $lectureIdDb = $isGlobal ? null : $lectureId;
    ...
    try {
      ...
      $stmt = $pdo->prepare("
        INSERT INTO lecture_codes (code, is_global, lecture_id, expires_at, created_by_admin_id)
        VALUES (?, ?, ?, ?, ?)
      ");
      ...
      for ($i=0; $i<$count; $i++) {
        ...
        while (true) {
          $tries++;
          if ($tries > $maxTries) throw new RuntimeException('تعذر توليد أكواد فريدة بعد محاولات كثيرة.');
      ...
      $pdo->commit();
      header('Location: lecture-codes.php?generated=1');
      exit;
    } catch (Throwable $e) {
...
if (($_POST['action'] ?? '') === 'delete') {
  ...
  if ($id <= 0) $error = 'طلب غير صالح.';
    ...
    try {
      $stmt = $pdo->prepare("DELETE FROM lecture_codes WHERE id=?");
      $stmt->execute([$id]);
      header('Location: lecture-codes.php?deleted=1');
      exit;
    } catch (Throwable $e) {
...
/* Messages */
if (isset($_GET['generated'])) $success = '✅ تم توليد أكواد المحاضرات بنجاح.';
...
/* =========================
   ...
   ========================= */
$q = trim((string)($_GET['q'] ?? ''));
$filterUsed = (string)($_GET['used'] ?? 'all');
...
$menu = [
  ...
  ['key' => 'lecture_codes', 'label' => 'اكواد المحاضرات', 'icon' => '🧾', 'href' => 'lecture-codes.php', 'active' => true],
...
<head>
  ...
  <title>اكواد المحاضرات - <?php echo h($platformName); ?></title>
  ...
  <link rel="stylesheet" href="assets/css/lecture-codes.css">
</head>
...
<body class="app" data-theme="auto">
  ...
  <div class="layout">
    ...
    <main class="main">
      <section class="lcodes-hero">
        <div class="lcodes-hero-title">
          <h1>🧾 أكواد المحاضرات</h1>
        </div>

        <div class="lcodes-metrics">
          <div class="metric">
      ...
      <section class="cardx" style="margin-top:12px;">
        ...
        <form method="post" class="lcodes-form" autocomplete="off" id="genForm">
          <input type="hidden" name="action" value="generate">

          <label class="field">
            <span class="label">نوع الكود</span>
            <select class="input2 select-pro" name="mode" id="modeSelect" required>
              <option value="lecture">🎯 كود لمحاضرة محددة</option>
              <option value="global">🌍 كود عام (يفتح كل المحاضرات)</option>
            </select>
          </label>
          ...
          <label class="field" id="courseField">
            ...
            <?php if (!$coursesList): ?>
              <div class="lcodes-hint">لا يوجد كورسات حالياً — أضف كورس أولاً.</div>
            <?php endif; ?>
          </label>

          <label class="field" id="lectureField">
            <span class="label">اسم المحاضرة</span>
            <select class="input2 select-pro" name="lecture_id" id="lectureSelect">
              <option value="0">— اختر المحاضرة —</option>
            </select>
          </label>

          <label class="field">
            <span class="label">عدد الاكواد</span>
      ...
      <section class="cardx" style="margin-top:12px;">
        <div class="cardx-head">
          ...
          <div class="cardx-actions">
            <a class="btn ghost" href="lecture-codes.php?export=1<?php
        ...
        <form method="get" class="lcodes-search" autocomplete="off">
          ...
          <div class="form-actions" style="margin:0">
            ...
            <a class="btn ghost" href="lecture-codes.php">مسح</a>
          </div>
        </form>
      </section>
      ...
      <section class="cardx" style="margin-top:12px;">
        ...
        <div class="table-wrap scroll-pro">
          <table class="table lcodes-table">
            <thead>
              <tr>
                <th>#</th>
                <th>الكود</th>
            ...
            <tbody>
              <?php if (!$codes): ?>
                <tr><td colspan="9" style="text-align:center">لا يوجد أكواد بعد.</td></tr>
              <?php endif; ?>

              <?php foreach ($codes as $lc): ?>
                <?php
                  $isGlobal = ((int)$lc['is_global'] === 1);
                  $isUsed = ((int)$lc['is_used'] === 1);
                  $expires = (string)($lc['expires_at'] ?? '');
                ?>
                <tr>
                  <td data-label="#"><?php echo (int)$lc['id']; ?></td>

                  <td data-label="الكود" class="code-cell">
                    <span class="code-text" dir="ltr"><?php echo h((string)$lc['code']); ?></span>
                    <button type="button" class="link info copy-btn" data-copy="<?php echo h((string)$lc['code']); ?>">📋 نسخ</button>
                  </td>
                  ...
                  <td data-label="النوع">
                    ...
                    <?php else: ?>
                      <span class="tagx green">🎯 محاضرة</span>
                    <?php endif; ?>
                  </td>

                  <td data-label="الكورس"><?php echo $isGlobal ? '—' : h((string)($lc['course_name'] ?? '')); ?></td>
                  <td data-label="المحاضرة"><?php echo $isGlobal ? '—' : h((string)($lc['lecture_name'] ?? '')); ?></td>

                  <td data-label="الانتهاء"><?php echo $expires !== '' ? h($expires) : '—'; ?></td>
                  ...
                  <td data-label="أضيف بتاريخ"><?php echo h((string)$lc['created_at']); ?></td>
                  ...
                  <td data-label="إجراءات" class="actions">
                    <form method="post" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الكود؟');">
                      ...
                      <input type="hidden" name="id" value="<?php echo (int)$lc['id']; ?>">
  ...
  <script>
    (function () {
      ...
      // Mode UI: show/hide course/lecture fields
      const modeSelect = document.getElementById('modeSelect');
      const courseField = document.getElementById('courseField');
      const lectureField = document.getElementById('lectureField');
      const courseSelect = document.getElementById('courseSelect');
      const lectureSelect = document.getElementById('lectureSelect');

      function syncMode() {
        const mode = modeSelect ? modeSelect.value : 'lecture';
        const isGlobal = (mode === 'global');

        courseField.style.display = isGlobal ? 'none' : '';
        lectureField.style.display = isGlobal ? 'none' : '';

        if (courseSelect) courseSelect.required = !isGlobal;
        if (lectureSelect) lectureSelect.required = !isGlobal;

        if (isGlobal) {
          if (courseSelect) courseSelect.value = '0';
          if (lectureSelect) lectureSelect.innerHTML = '<option value="0">— اختر المحاضرة —</option>';
        }
      }
      modeSelect && modeSelect.addEventListener('change', syncMode);
      syncMode();

      // Load lectures by course
      async function loadLectures(courseId) {
        ...
        try {
          const url = 'lecture_codes_lectures_api.php?course_id=' + encodeURIComponent(courseId);
