...
/* =========================
   ...
   =========================
   ✅ جدول exams (غير موجود في الدامب)
   ========================= */
$pdo->exec("
...
// all exam banks (we'll filter in UI by grade)
$banksList = $pdo->query("
  SELECT b.id, b.grade_id, b.name, g.name AS grade_name,
         (SELECT COUNT(*) FROM exam_questions q WHERE q.bank_id=b.id) AS bank_questions_count
...
function fetch_bank_questions_count(PDO $pdo, int $bankId): int {
  $stmt = $pdo->prepare("SELECT COUNT(*) AS c FROM exam_questions WHERE bank_id=?");
  $stmt->execute([$bankId]);
  $row = $stmt->fetch();
  return (int)($row['c'] ?? 0);
}

function exam_fetch_preview_payload(PDO $pdo, int $examId): array {
  // get exam + bank
  $stmt = $pdo->prepare("
    SELECT e.*,
    ...
    INNER JOIN exam_question_banks b ON b.id=e.bank_id
    WHERE e.id=? LIMIT 1
  ");
  $stmt->execute([$examId]);
  $e = $stmt->fetch();
  ...
  $bankId = (int)$e['bank_id'];
  $totalInBank = fetch_bank_questions_count($pdo, $bankId);

  $questionsTotal = (int)$e['questions_total'];
  $perStudent = (int)$e['questions_per_student'];
...
if (($_POST['action'] ?? '') === 'save_exam') {
  ...
  if ($name === '') $error = 'من فضلك اكتب اسم الامتحان.';
  elseif ($gradeId <= 0 || !isset($gradesMap[$gradeId])) $error = 'من فضلك اختر الصف الدراسي.';
  else {
    // validate bank belongs to grade + counts
    $stmt = $pdo->prepare("SELECT id, grade_id FROM exam_question_banks WHERE id=? LIMIT 1");
    $stmt->execute([$bankId]);
    $bank = $stmt->fetch();
    ...
    elseif ((int)$bank['grade_id'] !== $gradeId) $error = 'بنك الأسئلة لا يتبع الصف الدراسي المختار.';
      ...
      if ($bankCount <= 0) {
        ...
        if ($questionsTotal > $bankCount) $error = 'عدد أسئلة الامتحان لا يمكن أن يكون أكبر من عدد أسئلة البنك.';
        elseif ($questionsPerStudent > $questionsTotal) $error = 'عدد أسئلة الامتحان للطالب يجب ألا يزيد عن عدد أسئلة الامتحان.';
      }
    }
  }
  ...
  if (!$error) {
    try {
      if (!$isEdit) {
        ...
        header('Location: exams.php?added=1');
        exit;
      } else {
        $stmt = $pdo->prepare("
          UPDATE exams
...
if (($_POST['action'] ?? '') === 'delete_exam') {
  ...
  if ($id <= 0) $error = 'طلب غير صالح.';
    ...
    try {
      ...
      $error = 'تعذر حذف الامتحان.';
    }
  }
}

/* Messages */
if (isset($_GET['added'])) $success = '✅ تمت إضافة الامتحان بنجاح.';
if (isset($_GET['updated'])) $success = '💾 تم تعديل الامتحان بنجاح.';
if (isset($_GET['deleted'])) $success = '🗑️ تم حذف الامتحان بنجاح.';
...
if (($_GET['action'] ?? '') === 'preview_json') {
  ...
  if ($eid <= 0) {
    ...
    echo json_encode(['ok' => false, 'error' => 'exam_id is required'], JSON_UNESCAPED_UNICODE);
    exit;
  }

  $payload = exam_fetch_preview_payload($pdo, $eid);
  if (empty($payload['ok'])) {
...
$exams = $pdo->query("
  SELECT
    ...
    g.name AS grade_name,
    b.name AS bank_name,
    (SELECT COUNT(*) FROM exam_questions q WHERE q.bank_id=e.bank_id) AS bank_questions_count
  FROM exams e
  INNER JOIN grades g ON g.id=e.grade_id
  INNER JOIN exam_question_banks b ON b.id=e.bank_id
  ORDER BY e.id DESC
...
$menu = [
  ...
  ['key' => 'exams', 'label' => 'الامتحانات', 'icon' => '🧠', 'href' => 'exams.php', 'active' => true],
...
<head>
  ...
  <title>الامتحانات - <?php echo h($platformName); ?></title>
  ...
  <div class="layout">
    ...
    <main class="main">
        ...
        <div class="as-metrics">
          <div class="metric">
            ...
            <div class="metric-meta">
              <div class="metric-label">عدد الامتحانات</div>
              <div class="metric-val"><?php echo number_format($totalExams); ?></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="cardx" style="margin-top:12px;">
        <div class="cardx-head">
          <div class="cardx-title">
            ...
            <h2><?php echo $editRow ? 'تعديل امتحان' : 'إضافة امتحان جديد'; ?></h2>
          </div>
          <?php if ($editRow): ?>
            <a class="btn ghost" href="exams.php">إلغاء</a>
          <?php endif; ?>
        </div>

        <form method="post" class="as-form" autocomplete="off" id="examForm">
          ...
          <label class="field">
            <span class="label">وقت حل الامتحان (دقيقة)</span>
          ...
          <label class="field">
            <span class="label">عدد أسئلة الامتحان (من البنك)</span>
          ...
          <label class="field">
            <span class="label">عدد أسئلة الامتحان للطالب</span>
          ...
          <div class="form-actions" style="grid-column:1 / -1;">
            <button class="btn" type="submit" <?php echo (!$gradesList ? 'disabled' : ''); ?>>
              <?php echo $editRow ? '💾 حفظ التعديل' : '➕ إضافة الامتحان'; ?>
            </button>
          </div>
        </form>
      </section>
      ...
      <section class="cardx" style="margin-top:12px;">
        <div class="cardx-head">
          ...
          <div class="cardx-actions">
            <span class="pillx">عدد: <?php echo number_format($totalExams); ?></span>
          </div>
        </div>
        ...
        <div class="table-wrap scroll-pro">
          <table class="table as-table">
            ...
            <tbody>
              <?php if (!$exams): ?>
                <tr><td colspan="8" style="text-align:center">لا يوجد امتحانات بعد.</td></tr>
              <?php endif; ?>

              <?php foreach ($exams as $e): ?>
                <tr>
                  <td data-label="#"><?php echo (int)$e['id']; ?></td>
                  <td data-label="اسم الامتحان"><?php echo h((string)$e['name']); ?></td>
                  <td data-label="الصف"><span class="tagx purple">🏫 <?php echo h((string)$e['grade_name']); ?></span></td>
                  <td data-label="البنك">
                    <span class="tagx blue">🧠 <?php echo h((string)$e['bank_name']); ?></span>
                    <div class="mini">❓ في البنك: <?php echo number_format((int)$e['bank_questions_count']); ?></div>
                  </td>
                  <td data-label="الوقت"><span class="tagx orange">⏱️ <?php echo (int)$e['duration_minutes']; ?> دقيقة</span></td>
                  <td data-label="إجمالي/للطالب">
                    <span class="tagx green">📌 <?php echo (int)$e['questions_total']; ?></span>
                    <span class="tagx red">🧑‍🎓 <?php echo (int)$e['questions_per_student']; ?></span>
                  </td>
                  <td data-label="أضيف بتاريخ"><?php echo h((string)$e['created_at']); ?></td>
      ...
      <div class="as-modal" id="previewModal" aria-hidden="true">
        ...
        <div class="as-modal__card" role="dialog" aria-modal="true" aria-label="معاينة الامتحان">
          <div class="as-modal__head">
            <div class="as-modal__title">
              <div class="badge">👁️</div>
              <div>
                <h3 style="margin:0">معاينة الامتحان مثل الطالب</h3>
    ...
    (function () {
      ...
      function fillBanks(gradeId) {
        ...
        list.forEach(b => {
          ...
          opt.textContent = '🧠 ' + String(b.name) + ' (❓ ' + String(b.bank_questions_count) + ')';
      ...
      function renderPreview(payload) {
        ...
        previewMeta.textContent = `📝 ${e.name} • 🏫 ${e.grade_name} • 🧠 ${e.bank_name} • ⏱️ ${e.duration_minutes} دقيقة`;
        ...
        setActiveQuestion(0);
        startTimer(parseInt(e.duration_minutes || '0', 10) || 1);
      }

      async function fetchPreview(examId) {
        const url = `exams.php?action=preview_json&exam_id=${encodeURIComponent(examId)}`;
      ...
      document.addEventListener('click', async (e) => {
        ...
        try {
          btn.disabled = true;
          const payload = await fetchPreview(eid);
