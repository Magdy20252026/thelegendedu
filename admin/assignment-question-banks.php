...
if (($_POST['action'] ?? '') === 'create') {
  ...
  if ($name === '') $error = 'من فضلك اكتب اسم بنك الأسئلة.';
  elseif ($gradeId <= 0 || !isset($gradesMap[$gradeId])) $error = 'من فضلك اختر الصف الدراسي.';
  else {
    try {
      $stmt = $pdo->prepare("INSERT INTO assignment_question_banks (grade_id, name, created_by_admin_id) VALUES (?, ?, ?)");
      $stmt->execute([$gradeId, $name, $adminId > 0 ? $adminId : null]);

      header('Location: assignment-question-banks.php?added=1');
      exit;
    } catch (Throwable $e) {
...
if (($_POST['action'] ?? '') === 'update') {
  ...
  elseif ($name === '') $error = 'اسم بنك الأسئلة مطلوب.';
  elseif ($gradeId <= 0 || !isset($gradesMap[$gradeId])) $error = 'من فضلك اختر الصف الدراسي.';
  else {
    try {
      $stmt = $pdo->prepare("UPDATE assignment_question_banks SET grade_id=?, name=? WHERE id=?");
      $stmt->execute([$gradeId, $name, $id]);

      header('Location: assignment-question-banks.php?updated=1');
      exit;
    } catch (Throwable $e) {
...
if (($_POST['action'] ?? '') === 'delete') {
  ...
  if ($id <= 0) $error = 'طلب غير صالح.';
    ...
    try {
      $stmt = $pdo->prepare("DELETE FROM assignment_question_banks WHERE id=?");
      $stmt->execute([$id]);

      header('Location: assignment-question-banks.php?deleted=1');
      exit;
    } catch (Throwable $e) {
      $error = 'تعذر حذف بنك الأسئلة.';
    }
  }
}

/* Messages */
if (isset($_GET['added'])) $success = '✅ تمت إضافة بنك الأسئلة بنجاح.';
if (isset($_GET['updated'])) $success = '💾 تم تعديل بنك الأسئلة بنجاح.';
if (isset($_GET['deleted'])) $success = '🗑️ تم حذف بنك الأسئلة بنجاح.';
...
$banks = $pdo->query("
  SELECT
    ...
    (SELECT COUNT(*) FROM assignment_questions q WHERE q.bank_id = b.id) AS questions_count
...
$editRow = null;
if ($editId > 0) {
  $stmt = $pdo->prepare("SELECT id, grade_id, name FROM assignment_question_banks WHERE id=? LIMIT 1");
  $stmt->execute([$editId]);
...
<head>
  ...
  <title>بنوك أسئلة الواجبات - <?php echo h($platformName); ?></title>
    ...
    <main class="main">
      ...
      <section class="cardx" style="margin-top:12px;">
        <div class="cardx-head">
          ...
          <?php if ($editRow): ?>
            <a class="btn ghost" href="assignment-question-banks.php">إلغاء</a>
          <?php endif; ?>
        </div>
              ...
              <?php foreach ($banks as $b): ?>
                <tr>
                  ...
                  <td data-label="إجراءات" class="actions">
                    <a class="link info" href="assignment-questions.php?bank_id=<?php echo (int)$b['id']; ?>">➕ إدارة الأسئلة</a>
                    <a class="link" href="assignment-question-banks.php?edit=<?php echo (int)$b['id']; ?>">✏️ تعديل</a>
