...
/* =========================
   ...
   ========================= */
$pdo->exec("
  CREATE TABLE IF NOT EXISTS assignment_question_banks (
    id INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT fk_aqb_grade FOREIGN KEY (grade_id) REFERENCES grades(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
");
...
$pdo->exec("
  CREATE TABLE IF NOT EXISTS assignment_questions (
    ...
    CONSTRAINT fk_aq_bank FOREIGN KEY (bank_id) REFERENCES assignment_question_banks(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
");
...
$pdo->exec("
  CREATE TABLE IF NOT EXISTS assignment_question_choices (
    ...
    CONSTRAINT fk_aqc_question FOREIGN KEY (question_id) REFERENCES assignment_questions(id) ON DELETE CASCADE
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
");
...
$uploadDirAbs = __DIR__ . '/uploads/assignment_questions';
$uploadDirRel = 'uploads/assignment_questions';
...
if (($_POST['action'] ?? '') === 'delete_question') {
  ...
  if ($qid <= 0) $error = 'طلب غير صالح.';
    ...
    try {
      ...
      $stmt = $pdo->prepare("SELECT question_image_path FROM assignment_questions WHERE id=? AND bank_id=? LIMIT 1");
      ...
      $stmt = $pdo->prepare("SELECT choice_image_path FROM assignment_question_choices WHERE question_id=?");
      $stmt->execute([$qid]);
      $imgs = $stmt->fetchAll();
      foreach ($imgs as $im) {
      ...
      $stmt = $pdo->prepare("DELETE FROM assignment_questions WHERE id=? AND bank_id=?");
      $stmt->execute([$qid, $bankId]);

      header("Location: assignment-questions.php?bank_id={$bankId}&deleted=1");
      exit;
    } catch (Throwable $e) {
...
if (($_POST['action'] ?? '') === 'save_question') {
  ...
  // correct indices from form: correct_choices[] values = 1..choicesCount
  ...
  // validate correct indices count
  if (!$error) {
  ...
  if (!$error && $isEdit) {
    $stmt = $pdo->prepare("SELECT * FROM assignment_questions WHERE id=? AND bank_id=? LIMIT 1");
    $stmt->execute([$questionId, $bankId]);
    $oldQ = $stmt->fetch();
    if (!$oldQ) $error = 'السؤال غير موجود.';
    else $oldQuestionImage = (string)($oldQ['question_image_path'] ?? '');

    $stmt = $pdo->prepare("SELECT choice_index, choice_image_path FROM assignment_question_choices WHERE question_id=?");
  ...
  if (!$error && in_array($questionKind, ['image','text_image'], true)) {
    // image required on create; optional on edit (keep old)
    ...
    // questionKind text only => remove old image if exists (on edit)
    if ($isEdit && !empty($oldQuestionImage)) {
      $abs = __DIR__ . '/' . $oldQuestionImage;
      if (is_file($abs)) @unlink($abs);
    }
  ...
  $choices = []; // each: ['text'=>?,'image_path'=>?,'is_correct'=>0/1]
  if (!$error) {
    for ($i = 1; $i <= $choicesCount; $i++) {
      ...
      if ($needsImage) {
        ...
        // no image kind => delete old if exists on edit
        if ($isEdit && !empty($oldImg)) {
          $abs = __DIR__ . '/' . $oldImg;
          if (is_file($abs)) @unlink($abs);
        }
  ...
  if (!$error) {
    try {
      ...
      if (!$isEdit) {
        ...
        $stmtC = $pdo->prepare("
          INSERT INTO assignment_question_choices
        ...
        $stmt = $pdo->prepare("DELETE FROM assignment_question_choices WHERE question_id=?");
        $stmt->execute([$questionId]);
      ...
      header("Location: assignment-questions.php?bank_id={$bankId}&saved=1");
      exit;
    } catch (Throwable $e) {
...
$questions = $pdo->prepare("
  SELECT
    q.*,
    (SELECT COUNT(*) FROM assignment_question_choices c WHERE c.question_id = q.id) AS choices_actual
...
if ($editId > 0) {
  ...
  if ($editQuestion) {
    $stmt = $pdo->prepare("SELECT * FROM assignment_question_choices WHERE question_id=? ORDER BY choice_index ASC");
...
$menu = [
  ...
  ['key' => 'assignment_questions', 'label' => 'أسئلة الواجبات', 'icon' => '🗂️', 'href' => 'assignment-question-banks.php', 'active' => true],
...
<body class="app" data-theme="auto">
  ...
  <header class="topbar">
    ...
    <div class="top-actions">
      <a class="back-btn" href="assignment-question-banks.php">🗂️ الرجوع للبنوك</a>
    ...
    <main class="main">
      <section class="aq-hero">
        <div class="aq-hero-title">
          <h1>🗒️ أسئلة بنك: <?php echo h((string)$bank['name']); ?></h1>
      ...
      <section class="cardx" style="margin-top:12px;">
        <div class="cardx-head">
          ...
          <?php if ($editQuestion): ?>
            <a class="btn ghost" href="assignment-questions.php?bank_id=<?php echo (int)$bankId; ?>">إلغاء</a>
          <?php endif; ?>
        </div>
        ...
        <form method="post" class="aq-form" enctype="multipart/form-data" autocomplete="off" id="qForm">
          ...
          <label class="field" style="grid-column:1 / -1;" id="questionImageField">
            <span class="label">صورة السؤال</span>
              ...
              <?php foreach ($questions as $q): ?>
                <tr>
                  ...
                  <td data-label="إجراءات" class="actions">
                    ...
                    <a class="link" href="assignment-questions.php?bank_id=<?php echo (int)$bankId; ?>&edit=<?php echo (int)$q['id']; ?>">✏️ تعديل</a>
  ...
  <script>
    (function () {
      ...
      // =========================
      // ✅ Preview modal (mobile safe): event delegation
      // =========================
      ...
      async function fetchPreview(qid) {
        const url = `assignment-questions.php?bank_id=<?php echo (int)$bankId; ?>&action=preview_json&question_id=${encodeURIComponent(qid)}`;
      ...
      // ✅ delegation: works even if table/cards changed on mobile
